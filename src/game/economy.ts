...
export function upgradeList(state: GameState): UpgradeInfo[] {
  ...
  const scale = CONFIG.ships[state.ship.kind].refitScale;
  return [
    {
      key: 'spyglass',
    ...
    },
    {
      ...
      effect: `${r.stores < U.stores.length ? `+${Math.round(CONFIG.storesLevels[r.stores] * scale)} crew-days of provision space (${Math.round((CONFIG.storesLevels[r.stores] * scale) / Math.max(1, state.ship.crew))} days at this crew)` : 'Fully enlarged'}. A refit to this hull: it stays with the ship.`,
      level: r.stores,
    ...
    },
    {
      key: 'hold',
      name: 'Enlarged hold',
      effect: `${r.hold < U.hold.length ? `+${Math.round(CONFIG.holdLevels[r.hold] * scale)} units of cargo space` : 'Fully enlarged'}. A refit to this hull: it stays with the ship.`,
      level: r.hold,
